...
from typing import Dict, Iterable, List, Literal, Mapping, Optional, Set, Tuple, TypedDict, Union
...
class Gemma3MultiModalProjector(nn.Module):
    ...
    def __init__(self, config: PretrainedConfig):
        ...
        self.mm_input_projection_weight = nn.Parameter(
            torch.zeros(config.vision_config.hidden_size, config.hidden_size)
        )
...
class Gemma3ForConditionalGeneration(nn.Module):
    ...
    def pad_input_ids(self, input_ids: List[int], image_inputs: ImageInputs) -> List[int]:
